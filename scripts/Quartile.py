...
def DAIDataPriceFeed():
    payload = 'ETH'
    req = 'https://api.diadata.org/v1/quotation/' + payload
...
def NearAPICall(mention, royalty):
    imageURL = '''\\"''' + str(mention._json['message_create']['message_data']['entities']['urls'][0]['expanded_url']) + '''\\"'''
    tokenID = '''\\"''' + str(mention._json['id']) + '''\\"'''
    # royalty = [["benjiman.testnet", 2000], ["mike.testnet", 1000], ["josh.testnet", 500]]
    royalties = '''{ '''
    for entry in royalty:
        royalties = royalties + '''\\"''' + str(entry[0]) + '''.testnet\\":''' + str(entry[1]) + ''', '''
    ...
    cmd = '''near call royalty.rankjay.testnet nft_mint "{\\"token_id\\": ''' + tokenID + ''', \\"metadata\\": {\\"title\\": \\"NFT_TEST8\\", \\"description\\": \\"testing out the new approval extension of the standard\\", \\"media\\": ''' + imageURL + '''}, \\"receiver_id\\": \\"royalty.rankjay.testnet\\", \\"perpetual_royalties\\": ''' + royalties + '''}" --accountId royalty.rankjay.testnet --amount 0.1'''
...
def APIScrapper(mention, message):
    ...
    isNear = [word for word in message.split() if word == 'near']
    ...
    if len(isNear) == 0:
        ...
        jsonIdGetter = 'https://rinkeby.rarible.com/token/' + jsonIdGetter
        return jsonIdGetter

    else:
        royalty = [entry.split('$') for entry in [royaltySplit for royaltySplit in message.split() if '$' in royaltySplit]]
        jsonGetter = NearAPICall(mention, royalty)
        jsonIndexGetter = jsonGetter.find('explorer.testnet')
        jsonGetter = jsonGetter[jsonIndexGetter: len(jsonGetter)-2]
        print(jsonGetter)
...
def chainlinkAPICall():
    # subprocess.Popen("pushd ./chainlink/scripts", shell=True)
